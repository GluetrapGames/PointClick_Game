...
public class DropHeldItem : MonoBehaviour
{
	// Held item data
	[SerializeField]
	public HeldItemSlot _heldItemSlot;
	...
	[SerializeField]
	public GameObject _pickupParent;
	...
	{
		Instance = this;
		GameObject InventoryGroup = transform.parent.gameObject;
	...
	private void GetHeldItem()
	{
		...
		if (_heldItem == null || _heldItemType == ItemTypes.None ||
	...
	public void DropItem()
	{
		...
		GameObject pickupInstance =
			Instantiate(_pickupPrefab, _pickupParent.transform);
		pickupInstance.name = _heldItemType + " (Dropped)";
		...
		pickupInstance.GetComponent<PickUpScript>().pickupEvent =
			"player_pickup";
